...
function initGame(img, clicktarger) {
  ...
  $("#game_result span").text("Terminar");
  s = 0;
  ...
  if (isInGame) {
    alertBox(
      "Cambiar imagen",
      "¿El juego está en curso, estás seguro de que deseas cambiar la imagen y reiniciar?",
      function () {
        removeSelection();
        changeState(clickTarget);

        dividirImagen(img);
        $("#btn").text("Empezar");
        isInGame = false;
        puntaje = 0;
        $("#puntaje").text("Pasos: 0");
      }
    );
  } else {
...
function bindbtn() {
  ...
  $("#btn").click(function () {
    ...
    if (isInGame) {
      alertBox(
        "Restablecer juego",
        "El juego está en curso, ¿seguro que quieres restablecerlo?",
        function () {
          imgSplit(presentImg);
          rebackGame();
        }
...
function beginGamePc() {
  ...
  imgCells.bind("mousedown", function (e) {
    ...
    $(document).bind("mouseup", function (e3) {
      var cellIndex_2 = cellChangeIndex(
        e3.pageX - $("#imgArea").offset().left,
        e3.pageY - $("#imgArea").offset().top,
        cellIndex_1
      );
...
function beginGamePhone() {
  imgCells.on("touchstart", function (e) {
    ...
    $(document).on("touchend", function (e3) {
      var cellIndex_2 = cellChangeIndex(
        e3.changedTouches[0].pageX - $("#imgArea").offset().left,
        e3.changedTouches[0].pageY - $("#imgArea").offset().top,
        cellIndex_1
      );
      //console.log(cellIndex_2);
      if (cellIndex_1 == cellIndex_2) {
